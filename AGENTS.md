...
- Плагин совместим с Obsidian Mobile. Блок ` ```forgotten-notes ` использует CSS Grid (`repeat(auto-fill, minmax(160px, 1fr))`), чтобы карточки корректно переносились на узких экранах.
